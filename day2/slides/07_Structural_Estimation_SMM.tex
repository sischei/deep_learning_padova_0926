...
\begin{itemize}\setlength\itemsep{1pt}
    ...
    \item The valley and the basin: which moments identify
...
\begin{itemize}\itemsep4pt
  ...
  \item Simulate moments under common random numbers and minimize the criterion on the fly, for one parameter and for two
  \item Read a criterion surface: tell an identified basin from a flat valley, and confirm it with the moment Jacobian
\end{itemize}
\end{frame}
...
\textbf{Moments, and what moves them}
\begin{center}\footnotesize
\begin{tabular}{ll}
\toprule
$\mathrm{Std}(\Delta \log C_t)$ & persistence, weakly \\
$\mathrm{Corr}(\Delta \log C_t, \Delta \log C_{t-1})$ & persistence \\
$\mathrm{Corr}(\log Y_t, \log Y_{t-1})$ & persistence, directly \\
$\mathrm{mean}(I_t / Y_t)$ & the level of capital: $\beta$ \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.1cm}
{\footnotesize $\varrho$ sets how long shocks last, so autocorrelations identify it. $\beta$ sets the steady-state capital stock, $k^\star = \big(\alpha / (1/\beta - 1 + \delta)\big)^{1/(1-\alpha)}$, so a level moment identifies it and the dynamic ones barely do.}
\end{columns}
\end{frame}
...
\begin{frame}{Step 2: Moments Under Common Random Numbers}
\begin{center}
\includegraphics[width=0.98\linewidth]{figures/smm1_moments.pdf}
\end{center}
\vspace{-0.2cm}
{\footnotesize Notebook \texttt{07\_01}: 99 candidates from $0.50$ to $0.99$, each simulated for 2,000 periods from the same initial state under the same shocks, in 2 s. The dashed lines are the data moments. The output autocorrelation rises monotonically in $\varrho$ and identifies it on its own; the consumption-growth autocorrelation rises up to $\varrho \approx 0.9$ and falls beyond it, so near the truth it adds curvature but not sign; the standard deviation rises steeply only above $0.8$; the mean investment share is flat, as it should be, and is not used.}
\end{frame}

\begin{frame}{Step 3: The Criterion and the Estimate}
\begin{center}
\includegraphics[width=0.56\linewidth]{figures/smm1_criterion.pdf}
\end{center}
\vspace{-0.2cm}
{\footnotesize Notebook \texttt{07\_01}. The surrogate's criterion on 99 candidates, and the reference solver's on 8 candidates, one solve each. Grid minimum, then a bounded scalar minimizer: $\hat\varrho = $ 0.8793 against $0.90$. Two things keep the estimate off the truth, and the figure separates them: the 500-period sample (the reference criterion is not zero at $0.90$ either) and the surrogate's error (the gap between the two curves).}
\end{frame}
...
\begin{frame}{Two Criterion Surfaces: the Valley and the Basin}
\begin{center}
\includegraphics[width=0.96\linewidth]{figures/smm2_criterion_surfaces.pdf}
\end{center}
\vspace{-0.2cm}
{\footnotesize Notebook \texttt{07\_02}, 1,450 candidates under common random numbers. Left: the three dynamic moments only. A long valley along $\beta$: many pairs fit about equally well, and the minimizer's answer along it is arbitrary. Right: with the mean investment share added, the valley closes into a basin. \emphc{Identification is fixed by information, not by a better optimizer.}}
\end{frame}
...
\]
Near the minimum $Q \approx \delta u^\top M^\top M\, \delta u$, so $s_2^2$ is the criterion's curvature in its flattest direction and the last column of $V$ names that direction.
...
\begin{enumerate}\itemsep3pt
  ...
  \item Identification is a property of the moments. A flat valley in the criterion is either the moments or the surrogate; the reference check tells which, and the moment Jacobian puts a number on it.
\end{enumerate}

\vspace{0.2cm}
\begin{block}{Hands-on, \texttt{day2/code/07\_structural\_estimation/}}
\texttt{07\_01\_SMM\_One\_Parameter.ipynb} (4 min) and \texttt{07\_02\_SMM\_Two\_Parameters.ipynb} (6 min), teaching presets stored; set \texttt{RUN\_MODE = "smoke"} in class. Try: change the true $\varrho$; drop a moment; shorten the observed sample to $T = 100$ and watch the criterion's minimum move.
\end{block}
\end{frame}
