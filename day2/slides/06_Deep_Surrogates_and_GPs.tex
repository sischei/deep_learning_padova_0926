...
\vspace{0.1cm}
{\footnotesize No training loop, no learning rate, no architecture: the whole model is the kernel and one Cholesky factorization. Compare Algorithm 2.1 in Rasmussen \& Williams (2006); notebook \texttt{06\_02} builds this and checks it against \texttt{sklearn}.}
\end{frame}
...
\vspace{2pt}
\emphc{But do not oversell it.} Notebook \texttt{06\_02} runs this against a uniform design and they finish \emph{level} (MAE $0.0234$ vs $0.0237$ at $N=25$). For a \emph{stationary} kernel the posterior variance depends on the design alone, so on an interval uncertainty sampling essentially \emph{is} a space-filling design. It earns its keep where no such design exists: \emphc{irregular domains, ARD kernels, and dimensions where a grid is impossible}.}
\end{frame}
...
\small
Notebooks in \texttt{../code/06\_surrogates\_and\_gps} and \texttt{../code/07\_structural\_estimation}: PyTorch $+$ scikit-learn, CPU-only, self-contained. \texttt{06\_01} and \texttt{06\_02} ship on \texttt{RUN\_MODE = "smoke"}; raise it once a run works.

\vspace{0.15cm}
\begin{itemize}\itemsep4pt
  \item \textbf{A deep surrogate, end to end} \emph{(${\approx}3.5$ min)}. Black--Scholes over $(S, K, T, \sigma, r)$, then implied-vol inversion by root-finding \emph{and} by gradient descent. \emphc{Read its timing table:} on this toy the surrogate \emph{loses} to numpy: the ``price one label first'' rule biting. \hfill{\footnotesize\texttt{06\_01}}
  \item \textbf{GP regression and active learning} \emph{(${\approx}30$ s)}. A GP from scratch in numpy, checked against \texttt{sklearn}; then the BAL loop and its budget sweep. \hfill{\footnotesize\texttt{06\_02}}
  \item \textbf{Estimating $\varrho$ by SMM} \emph{(${\approx}10$ s)}. Train with $\varrho$ as a pseudo-state, simulate under common random numbers, recover the parameter. \hfill{\footnotesize\texttt{07\_01}}
  \item \textbf{The identification ridge} \emph{(${\approx}10$ s)}. Free $\beta$ as well, watch the valley appear, and read the moment Jacobian's singular values. \hfill{\footnotesize\texttt{07\_02}}
\end{itemize}

\vspace{0.15cm}
{\footnotesize \textbf{If you only run one:} \texttt{06\_02}. It builds a GP from scratch, and its budget sweep is the measurement behind the honest version of the active-learning claim.}
\end{frame}
...
\vspace{0.5cm}
{\footnotesize
\textbf{Next up, Session 7:} \emph{Structural estimation via the simulated method of moments}, where the surrogate we just built removes the inner-loop model solve.
}
