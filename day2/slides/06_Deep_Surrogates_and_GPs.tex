...
\begin{tikzpicture}[
    ...
    \draw[uzhblue!30, line width=2pt] (0,0.45) -- (0,-3.52);
    ...
    \node[font=\tiny\bfseries, text=uzhblue, anchor=south east] at (8.85,0.34) {you are here $\blacktriangledown$};
    \node[time] at (-0.35,-0.62) {09:45--10:30};
    \node[session] at (0.25,-0.62) {Structural estimation via SMM};
    \fill[uzhblue] (0,-0.62) circle (3.5pt);
    \node[pause] at (0.25,-1.24) {coffee break};
    \fill[uzhblue!30] (0,-1.24) circle (2pt);
    \node[time] at (-0.35,-1.86) {10:45--12:00};
    \node[session] at (0.25,-1.86) {PINNs I: foundations};
    \fill[uzhblue] (0,-1.86) circle (3.5pt);
    \node[pause] at (0.25,-2.48) {lunch break};
    \fill[uzhblue!30] (0,-2.48) circle (2pt);
    \node[time] at (-0.35,-3.10) {14:30--16:00};
    \node[session] at (0.25,-3.10) {PINNs II: applications, hands-on, wrap-up};
    \fill[uzhblue] (0,-3.10) circle (3.5pt);
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}{Roadmap of This Lecture (45 min)}
\small
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{I.\ What is a surrogate?}~~\mbox{\small (10 min)}
...
\vspace{0.4em}
\textbf{II.\ GP regression}~~\mbox{\small (23 min)}
\begin{itemize}\setlength\itemsep{1pt}
...
\column{0.48\textwidth}
\textbf{III.\ GP or network?}~~\mbox{\small (9 min)}
\begin{itemize}\setlength\itemsep{1pt}
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.74\linewidth]{figures/surrogate_primer_surface.pdf}
\end{center}
\vspace{-0.25cm}
{\footnotesize Notebook \texttt{06\_01}. Left: the GP surrogate's mean capital from \emphc{40 solves} (white dots); a $30 \times 30$ grid would have cost 25 minutes. Right: its posterior standard deviation, largest away from the design. On 60 fresh solves the GP's mean error is $1.1\cdot10^{-3}$ of the quantity's range, the network's $3.9\cdot10^{-3}$, both fit to the same 40 labels.}
\end{frame}
...
\end{itemize}
\vspace{0.1cm}
{\footnotesize Session 7: \texttt{07\_01}, \texttt{07\_02}; then compared with GPs of the moment map.}
\end{columns}
...
\begin{frame}{Few Labels: the GP Is Better, and Calibrated}
\vspace{-0.2cm}
\begin{center}
\includegraphics[height=0.40\textheight]{figures/surrogate_primer_error_vs_n.pdf}
\end{center}
\vspace{-0.4cm}
\footnotesize
\begin{enumerate}\itemsep2pt
  \item \textbf{Accuracy.} Same labels, same 60 test solves. At 40 labels the GP's mean error is $1.1\cdot10^{-3}$ of the range, the network's $3.9\cdot10^{-3}$; at 160 the GP is at $1.0\cdot10^{-4}$, the network still at $3.0\cdot10^{-3}$.
  \item \textbf{Calibration.} From 40 labels on, the GP's 95\% band contains the truth at 92 to 98\% of the test points: the error bar can be trusted, and it says where the surrogate is unsure.
  \item \textbf{Why.} The GP passes through the labels exactly under a smoothness prior; with few labels that prior beats the network's flexibility, which is limited here by its training, not its size. With thousands of labels and many inputs the balance flips (Part I, Session 7).
\end{enumerate}
\vspace{-0.1cm}
{\scriptsize Notebook \texttt{06\_01}. GP: Matérn 5/2, one length scale per input, marginal likelihood. Network: $2\times32$ tanh, Adam, 3{,}000 epochs.}
\end{frame}
...
\begin{frame}{Active Learning: Let the Band Choose the Next Label}
\small
\vspace{-0.15cm}
With the kernel fixed, the posterior variance depends on the design alone, so it can be consulted \emph{before} the next label is paid for: \textbf{query where the band is widest, refit, repeat.}
\vspace{-0.1cm}
\begin{center}
\includegraphics[width=0.78\linewidth]{figures/gp_active_learning.pdf}
\end{center}
\vspace{-0.45cm}
{\scriptsize Notebook \texttt{06\_02}: $f(x) = 1/(1+25x^2)$ on $[-1,1]$, squared-exponential kernel with $\ell = 0.3$ fixed, starting from the two endpoints; the dotted line is the next query.}
\vspace{0.05cm}
\begin{itemize}\itemsep1pt
  \item \textbf{What it does.} After the endpoints it takes the centre, then the quarter points: the widest band is always the largest gap in the design. After 8 points: $\{-1, -0.76, -0.5, 0, 0.23, 0.51, 0.76, 1\}$.
  \item \textbf{What it does not do on an interval.} A uniform design with 8 points is already that, and does slightly better (mean absolute error $0.043$ against $0.072$). The loop pays where no uniform design exists: irregular domains, unequal length scales, many inputs (Scheidegger \& Bilionis, 2019).
\end{itemize}
\end{frame}
