...
\emphc{interior} Euler equation.  Part III keeps $\lambda$ and adds a second multiplier.}
\end{frame}
...
\vspace{-2pt}
{\scriptsize\emphc{Left:} monitor loss and mean train-step loss per segment, $\log_{10}$ scale; the cloud is re-simulated each segment.\\
\emphc{Right:} mean savings fraction by cohort against the closed form $\beta_h$, matched to $1.0\times 10^{-5}$.}
\vspace{-2pt}
\end{block}
\vspace{-5pt}
{\scriptsize \texttt{04\_01} at \texttt{RUN\_MODE = "teaching"}, eleven minutes on a laptop CPU; mean relative Euler error $2.6\times10^{-4}$.  Use \texttt{"smoke"} ($\sim$2.5 min) in class.}
\end{frame}
...
\footnotesize
\centering
\includegraphics[height=2.35cm]{figures/olg56_lifecycle_a.pdf}\hfill
\includegraphics[height=2.35cm]{figures/olg56_lifecycle_consumption_by_shock.pdf}
...
\qquad\text{vs}\qquad
\underbrace{1.25\times A = 70}_{\text{the sampling box}},\quad
\underbrace{K \in [10,\,27]}_{\text{a healthy cloud}}
\]
\vspace{-6pt}
\begin{center}
\emphc{Eight times the widest state the notebook itself calls feasible.  It never fired once, in any run.}
\end{center}
\vspace{-2pt}
