...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{04\_01\_OLG\_Analytic\_DEQN\_persistent.ipynb}; companion ablation \texttt{04\_04\_OLG\_Analytic\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\vspace{-2pt}
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{04\_02\_OLG\_Benchmark\_DEQN\_persistent.ipynb}; companion ablation \texttt{04\_05\_OLG\_Benchmark\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\end{frame}
...
\scriptsize
\emphc{Open notebook \texttt{04\_01\_OLG\_Analytic\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; the exogenous-sampling companion is \texttt{04\_04\_OLG\_Analytic\_DEQN\_exogenous.ipynb}).  It implements the model, equilibrium conditions, loss, FREE map, architecture, training, and validation covered in Part II.
\vspace{-2pt}
...
\vspace{2pt}
The actual training run, DEQN-vs-analytic comparison, and Euler-error histogram are produced by \texttt{04\_01\_OLG\_Analytic\_DEQN\_persistent.ipynb} (\S 8).  Production target in AGS (2022): mean relative Euler error $\lesssim 10^{-3}$.
\end{block}
\end{frame}
...
\begin{block}{Sizes from \texttt{04\_02\_OLG\_Benchmark\_DEQN\_persistent.ipynb}}
\centering
\begin{tabular}{ll}
\toprule
...
\vspace{-3pt}
For each aggregate state $z \in \{1,2,3,4\}$ the trained policy yields a lifecycle consumption profile $c^s(z)$ over ages $s=25,\ldots,80$, read off a long ergodic simulation from \texttt{04\_02\_OLG\_Benchmark\_DEQN\_persistent.ipynb}.  (The notebook ships pooled lifecycle bands and residual diagnostics; the per-shock breakdown is a small add.)
\end{block}
...
\vspace{-2pt}
\textbf{Hands-on: \texttt{04\_02\_OLG\_Benchmark\_DEQN\_persistent.ipynb}} (persistent-simulation primary; \texttt{04\_05\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} is the feedback-free ablation, same model and loss, different training cloud).  \S 1--4 match III.1--III.5; \S 5--7 = training and policy stability; \S 8 = plots/diagnostics of III.10--III.12.  Teaching runtime ($\sim$200 segments, 128-wide): a few minutes.
\end{frame}
...
\footnotesize
\textbf{What we just did.}  Cohort structure changed the \emph{size} of the residual system --- one
Euler equation per generation, stacked into a single Lagrangian primitive --- but not the training
principle. Borrowing and collateral constraints entered through product-form KKT residuals, and
two-tape automatic differentiation meant no residual was ever derived by hand.
...
loop --- and the point where a tensor-product grid is no longer an option at all.
