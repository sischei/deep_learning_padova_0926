...
laptop. The network should report $\hat\sigma = 0.4118$ at every state the economy visits, and drift away from it \emph{outside} that set, because the loss is only ever evaluated where the policy goes (Session 2, ``Residual training only tests states the policy visits'').  That is why training uses a simulated cloud.}
\end{frame}
...
\emphc{Right:} mean savings fraction by cohort against the closed form $\beta_h$; the DEQN matches it to $1.4\times 10^{-4}$.
\end{block}
\vspace{-4pt}
{\scriptsize From \texttt{04\_01}, \texttt{RUN\_MODE = "teaching"}: about 45 minutes on a laptop CPU, so the stored outputs are that run; use \texttt{"smoke"} ($\sim$30 s) in class.}
\end{frame}
...
Open \texttt{04\_01\_OLG\_Analytic\_DEQN\_persistent.ipynb}.  Run \texttt{RUN\_MODE = "smoke"} ($\sim$30 s) now; the stored outputs are already the converged \texttt{"teaching"} run ($\sim$45 min on a laptop CPU), and \texttt{"production"} chases AGS Table~3 accuracy.  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch (the persistent primary uses \texttt{"simulation"}, the companion \texttt{04\_04\_OLG\_Analytic\_DEQN\_exogenous.ipynb} uses \texttt{"exogenous"}); the model and loss are identical, only the training cloud differs.
\end{block}
\end{frame}
...
\begin{frame}{III.4 Solved: Lifecycle Profiles from the Paper's Network}
\footnotesize
\centering
\includegraphics[height=2.8cm]{figures/olg56_lifecycle_a.pdf}\hfill
\includegraphics[height=2.8cm]{figures/olg56_lifecycle_consumption_by_shock.pdf}
...
\begin{itemize}\itemsep0pt
    ...
    \item \texttt{-\,-train\_from\_scratch} reproduces the two-stage schedule: a GPU job, not a laptop one,
...
% --- III.5 what a failed run looks like ---------------------------------
\begin{frame}{III.5 What a \emphc{Failed} Run Looks Like}
\footnotesize
Preparing this lecture, I tried to shrink the benchmark to a laptop-sized preset so you could train it
in class.  It does not work, and the way it fails is worth more than the run would have been.
Four attempts, three diverged:
...
\begin{tabular}{lrr}
\toprule
Diagnostic & healthy (Part II) & the runs that failed \\
\midrule
$\log_{10}\Loss$                          & $-5.1$                & $+7$ to $+11$ \\
aggregate $K$ across the cloud            & a spread of states    & runs off to $130$, then a single point \\
...
\end{center}
\vspace{-3pt}
\begin{alertblock}{How to read it}
\vspace{-3pt}\begin{itemize}\itemsep0pt
    \item It looked \emph{fine} for twenty segments, then $K$ split away and never came back.  A loss
          that is falling tells you nothing on its own.
    \item \emphc{Watch the state distribution, not the loss.}  The simulated cloud is the training set;
          once it runs away, the residual is being minimised somewhere the economy never goes.
    \item Identical settings gave both a converged run and a diverged one: on CPU the arithmetic is not
          bit-reproducible, and this problem sits close enough to the edge for that to decide the outcome.
\end{itemize}
\vspace{-3pt}
\end{alertblock}
\vspace{-2pt}
{\scriptsize This is what Session 3's architecture search and loss balancing are for, and why AGS (2022)
runs a two-stage schedule on a GPU.}
\end{frame}
