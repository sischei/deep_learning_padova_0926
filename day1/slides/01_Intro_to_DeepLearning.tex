...
% ---------- Title ----------
\title[Intro to DL -- Padova 2026]%
{Deep Learning for Solving Dynamic Models}
\subtitle{Session 1: Introduction to machine learning and deep learning}
...
\begin{tikzpicture}[
    session/.style={rectangle, rounded corners=4pt, draw=uzhblue, thick, fill=softblue!15,
                    minimum width=8.6cm, minimum height=0.5cm, align=left, font=\footnotesize,
                    inner xsep=10pt, inner ysep=2pt, anchor=west},
    ...
    \draw[uzhblue!30, line width=2pt] (0,0.4) -- (0,-5.46);
    \node[time] at (-0.35,0.00) {09:30--10:30};
    \node[now] at (0.25,0.00) {Introduction to machine learning and deep learning};
    \fill[uzhblue] (0,0.00) circle (3.5pt);
    \node[font=\tiny\bfseries, text=uzhblue, anchor=west] at (8.95,0.00) {$\blacktriangleleft$ you are here};
    \node[time] at (-0.35,-0.56) {10:30--11:00};
    \node[session] at (0.25,-0.56) {Deep Equilibrium Nets I: motivation and the method};
    \fill[uzhblue] (0,-0.56) circle (3.5pt);
    \node[pause] at (0.25,-1.12) {coffee break};
    \fill[uzhblue!30] (0,-1.12) circle (2pt);
    \node[time] at (-0.35,-1.68) {11:15--12:10};
    \node[session] at (0.25,-1.68) {Deep Equilibrium Nets II: Brock--Mirman, hands-on};
    \fill[uzhblue] (0,-1.68) circle (3.5pt);
    \node[time] at (-0.35,-2.24) {12:10--12:45};
    \node[session] at (0.25,-2.24) {Neural architecture search};
    \fill[uzhblue] (0,-2.24) circle (3.5pt);
    \node[pause] at (0.25,-2.80) {lunch break};
    \fill[uzhblue!30] (0,-2.80) circle (2pt);
    \node[time] at (-0.35,-3.36) {14:00--14:30};
    \node[session] at (0.25,-3.36) {Loss balancing};
    \fill[uzhblue] (0,-3.36) circle (3.5pt);
    \node[time] at (-0.35,-3.92) {14:30--15:30};
    \node[session] at (0.25,-3.92) {Overlapping generations with DEQNs};
    \fill[uzhblue] (0,-3.92) circle (3.5pt);
    \node[pause] at (0.25,-4.48) {coffee break};
    \fill[uzhblue!30] (0,-4.48) circle (2pt);
    \node[time] at (-0.35,-5.04) {15:45--17:00};
    \node[session] at (0.25,-5.04) {IRBC with DEQNs; what to run tonight};
    \fill[uzhblue] (0,-5.04) circle (3.5pt);
\end{tikzpicture}
\end{center}
\end{frame}

% --- Slide 3: Roadmap ---
\begin{frame}{Roadmap of This Lecture (60 min)}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Setting the Stage}~~\mbox{\small (8 min)}
\begin{itemize}
...
\vspace{0.5em}
\textbf{Machine Learning Foundations}~~\mbox{\small (10 min)}
\begin{itemize}
...
\vspace{0.5em}
\textbf{From Perceptrons to Deep Nets}~~\mbox{\small (10 min)}
\begin{itemize}
    \item Neurons, activations, universal approximation
\end{itemize}

\column{0.48\textwidth}
\textbf{Training Neural Networks}~~\mbox{\small (16 min)}
\begin{itemize}
...
\vspace{0.5em}
\textbf{Generalization \& Regularization}~~\mbox{\small (8 min)}
\begin{itemize}
    \item Overfitting, early stopping, dropout
\end{itemize}

\vspace{0.5em}
\textbf{Outlook \& Practice}~~\mbox{\small (8 min)}
\begin{itemize}
...
\vspace{0.7cm}
\begin{center}
\emphc{This course focuses on the last category:} deep learning as a \emph{computational} tool for economic modeling.
\end{center}
\end{frame}
...
\vspace{0.5cm}
{\small The neural network tools from this lecture are the building blocks for all of these paradigms.}
\end{frame}
...
\begin{itemize}\itemsep1pt
    \item {\footnotesize $b \equiv w_0$ is the \textbf{bias} (shifts the activation)}
    \item {\footnotesize $g(\cdot)$ is the \textbf{activation function}.  Rosenblatt used a
          \textbf{step}: output $1$ if the sum is positive, else $0$}
    \item {\footnotesize The weights are now \emphc{learned from data}, not fixed by hand}
\end{itemize}

\end{columns}

\vspace{0.25cm}
{\scriptsize \emphc{Without $g$ this is just a linear model.}  The step has no useful derivative, so smooth activations come next.
\emphc{Minsky \& Papert (1969):} one perceptron cannot represent XOR, which is exactly why we need hidden layers.}
\end{frame}
...
\vspace{0.25cm}
\begin{exampleblock}{The hinge of this whole course}
...
\small
\begin{block}{Next, Session 2: Deep Equilibrium Nets}
\small
...
\vspace{0.1cm}
\begin{center}
{\footnotesize\textbf{Materials:}~\href{https://github.com/sischei/deep_learning_padova_0926}{course GitHub repository}~~$\cdot$~~\textbf{Platform:}~\href{https://nuvolos.cloud}{nuvolos.cloud} (no local setup required)}
\end{center}
\end{frame}
...
\vspace{0.6cm}
{\small Materials:}\\[3pt]
{\footnotesize\href{https://github.com/sischei/deep_learning_padova_0926}{github.com/sischei/deep\_learning\_padova\_0926}}
\end{center}
\end{columns}
\end{frame}
