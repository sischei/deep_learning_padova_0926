...
\begin{tikzpicture}[scale=0.74, transform shape,
    ...
    % The same network at next period's state, for the Euler equation
    \node[block, dashed, minimum width=2.6cm, minimum height=0.7cm, font=\scriptsize] (next) at (8.8,1.7)
          {$\pi_\theta(\x_{t+1})$: same weights};

    % Residuals: each one uses every output
    \node[resbase, draw=softblue, fill=softblue!10] (rA) at (12.3, 1.5) {Euler residual $R_1$};
    \node[resbase, draw=softorange, fill=softorange!10] (rB) at (12.3, 0) {Labour condition $R_2$};
    \node[resbase, draw=softgreen, fill=softgreen!10] (rC) at (12.3,-1.5) {Constraint / KKT $R_3$};

    % Losses
    \node[lossbase, draw=softblue, fill=softblue!15] (lossA) at (15.5, 1.5) {$\mathcal{L}_1$};
    \node[lossbase, draw=softorange, fill=softorange!15] (lossB) at (15.5, 0) {$\mathcal{L}_2$};
    \node[lossbase, draw=softgreen, fill=softgreen!15] (lossC) at (15.5,-1.5) {$\mathcal{L}_3$};

    % Arrows
    \draw[->, thick] (input) -- (h1);
    \draw[->, thick] (h1) -- (h2);
    \draw[->, thick] (h2) -- (pol);
    \draw[->, thick] (pol.east) -- ++(0.3,0) |- (rA.west);
    \draw[->, thick] (pol.east) -- (rB.west);
    \draw[->, thick] (pol.east) -- ++(0.3,0) |- (rC.west);
    \draw[->, thick, dashed] (next.east) -- (rA.west);
    \draw[->, thick] (rA) -- (lossA);
    \draw[->, thick] (rB) -- (lossB);
    \draw[->, thick] (rC) -- (lossC);
    ...
    \draw[decorate, decoration={brace, amplitude=8pt, mirror},
          uzhgreydark, thick] (10.6,-2.1) -- (14.0,-2.1)
