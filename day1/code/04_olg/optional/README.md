...
| loss, training states | 8.0e-03 | 1.7e-02 |
| loss, simulated (ergodic) states | 8.2e-03 | **2.1e+01** |
| mean rel. Euler error, capital | 4.1% | 7.5% on its box, **14.3%** on the ergodic set |
| bond market clearing, share of output | 0.14% | 0.12% on its box, **7.1%** on the ergodic set |
| share of states with c ≤ 0 | 0% | 0% |
| policy drift | PASS | PASS |

`04_05`, trained on a fixed box, looks healthy on that box and is **1180× worse** on the states the
economy actually visits, with a bond market off by 7% of output. `04_02`, trained on its own simulation,
scores the same on both sets of states. At 56 cohorts the ergodic set is not known in advance and the box
misses it; at six cohorts (`04_04`) it is, and the box works.
...
sampling box tops out at `EXOGENOUS_K_HIGH * N_AGES` = 70, and a healthy set of simulated states sits at
aggregate capital of roughly 10 to 27. A guard at 560 is eight times the widest state the notebook is
willing to call feasible: it never fired in any run, and its `repairs=0` counter looked the same as a
healthy run's. Tied to the sampling box, training is stable.
...
* the runaway guard is doing continuous work. It first fires at segment 30 and then resamples
  **about 131 of the 1024 states in every segment, 13% of the training set, for the remaining 2970
  segments**. Aggregate capital on the simulated states sits at the bound (69.999 against 70.0).

So the policy is being held inside the feasible set rather than settling there, and the states it
trains on are partly an artifact of the repair mechanism. Stable is not the same as solved. For the
